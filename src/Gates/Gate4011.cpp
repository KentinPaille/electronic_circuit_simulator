...
enum Gate4011_LAYOUT {
    ...
    INPUT7 = 12,
    INPUT8 = 13,
    Gate4011_LAYOUT_MAX = INPUT8
};

Gate4011::Gate4011(std::string name) : AComponent(name, "Gate4011") {
...
Tristate Gate4011::compute(std::size_t pin) {
    if (pin == OUTPUT1)
        return NANDLogic(OUTPUT1, INPUT1, INPUT2);
    if (pin == OUTPUT2)
        return NANDLogic(OUTPUT2, INPUT3, INPUT4);
    if (pin == OUTPUT3)
        return NANDLogic(OUTPUT3, INPUT5, INPUT6);
    if (pin == OUTPUT4)
        return NANDLogic(OUTPUT4, INPUT7, INPUT8);
...
Tristate Gate4011::NANDLogic(std::size_t output, std::size_t input1, std::size_t input2) {
...
void Gate4011::setLink(std::size_t pin, IComponent &other, std::size_t otherPin) {
    if (pin > Gate4011_LAYOUT_MAX || pin <= 0) {
