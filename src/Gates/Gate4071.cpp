...
enum Gate4071_LAYOUT {
    ...
    INPUT7 = 12,
    INPUT8 = 13,
    Gate4071_LAYOUT_MAX = INPUT8
};

Gate4071::Gate4071(std::string name) : AComponent(name, "Gate4071") {
...
Tristate Gate4071::compute(std::size_t pin) {
    if (pin == OUTPUT1)
        return ORLogic(OUTPUT1, INPUT1, INPUT2);
    if (pin == OUTPUT2)
        return ORLogic(OUTPUT2, INPUT3, INPUT4);
    if (pin == OUTPUT3)
        return ORLogic(OUTPUT3, INPUT5, INPUT6);
    if (pin == OUTPUT4)
        return ORLogic(OUTPUT4, INPUT7, INPUT8);
...
Tristate Gate4071::ORLogic(std::size_t output, std::size_t input1, std::size_t input2) {
...
void Gate4071::setLink(std::size_t pin, IComponent &other, std::size_t otherPin) {
    if (pin > Gate4071_LAYOUT_MAX || pin <= 0) {
